...
const debug = (0, _debug2.default)('node-tnef');
...
function parseBuffer(data, callback) {
    var arr = [...data];
        ...
        DecodeFile(fullPath).then(result => {
            ...
            if (result && result.Attachments && result.Attachments.length > 0) {
                ...
                if (result.Body || result.BodyHTML) {
                    _fs2.default.writeFile(_path2.default.join(processedPath, 'htmlbody.html'), new Buffer(result.Body || result.BodyHTML), err => {
                        debug('ERROR' + err.message);
                        reject(err);
                    });
                }
